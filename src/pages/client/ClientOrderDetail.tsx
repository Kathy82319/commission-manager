...
import '../../styles/ClientOrderDetail.css'; // 🌟 引入外部 CSS
...
export function ClientOrderDetail() {
  ...
  const renderClientStageBox = (title: string, stageKey: string, isPassed: boolean) => {
    ...
    if (isFreeMode) { statusText = ''; } 
      ...
      headerBg = '#e6f4ea'; statusColor = '#1e8e3e';
      statusText = isFinal ? '✓ 已同意，合約結案' : '✓ 繪師已推進下一階段';
    } else if (sub) {
      headerBg = '#fce8e6'; statusColor = '#d93025';
              ...
              <div style={{ marginTop: '20px', display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                ...
                {shouldShowActionButtons && (
                  <>
                    ...
                    <button onClick={() => handleReview(stageKey, 'reject')} disabled={isProcessing} style={{ padding: '10px 20px', backgroundColor: '#FFF', color: '#d93025', border: '1px solid #d0d8e4', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', flex: '1 1 auto' }}>退回修改</button>
      ...
      {parsedChanges && (
        <div className="change-overlay">
          <div className="change-modal">
            <h3 style={{ color: '#e11d48', marginTop: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>⚠️ 繪師提出了規格異動申請</h3>
            <p style={{ color: '#556577', fontSize: '14px', marginBottom: '12px' }}>繪師希望調整委託單內容，請確認以下項目：</p>
            <div style={{ backgroundColor: '#f1f5f9', padding: '16px', borderRadius: '12px', fontSize: '14px', color: '#475569', marginBottom: '24px', whiteSpace: 'pre-wrap', maxHeight: '200px', overflowY: 'auto', border: '1px solid #d0d8e4' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => handleReviewChange('approve')} style={{ flex: 1, padding: '14px', backgroundColor: '#1e8e3e', color: '#FFF', borderRadius: '12px', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px' }}>同意並更新</button>
        ...
        <div style={{ backgroundColor: '#e8ecf3', padding: '20px', borderRadius: '0 0 16px 16px', minHeight: '400px' }}>
          {activeTab === 'main' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <input 
                      ...
                      style={{ flex: '1 1 200px', padding: '10px', borderRadius: '8px', outline: 'none', border: isSavedAndNotEmpty ? '1px solid #e2e8f0' : '1px solid #94a3b8', backgroundColor: isSavedAndNotEmpty ? '#f1f5f9' : '#FFFFFF', color: '#475569' }}
                    />
                    <button 
                      onClick={handleSaveTitle} disabled={saveStatus !== 'idle'}
                      style={{ padding: '10px 20px', color: '#FFF', border: 'none', borderRadius: '8px', cursor: saveStatus === 'idle' ? 'pointer' : 'default', fontWeight: 'bold', backgroundColor: saveStatus === 'success' ? '#1e8e3e' : '#556577', flex: '1 1 auto', minWidth: '90px' }}
                    >
              ...
              <div className="section-box" style={{ marginBottom: '0' }}>
                <h3 style={{ fontSize: '16px', color: '#475569', margin: '0 0 12px 0', borderBottom: '1px solid #e8ecf3', paddingBottom: '8px' }}>委託規格</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px', fontSize: '14px', color: '#556577' }}>
                  ...
                  <div style={{ gridColumn: '1 / -1', marginTop: '4px', borderTop: '1px dashed #d0d8e4', paddingTop: '12px', fontSize: '16px', color: '#4A7294' }}>
            ...
            <div>
              {logs.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {logs.map(log => (
                    <div key={log.id} style={{ padding: '12px', backgroundColor: '#FFFFFF', borderRadius: '8px', borderLeft: log.actor_role === 'artist' ? '4px solid #4E7A5A' : '4px solid #4A7294', borderTop: '1px solid #d0d8e4', borderRight: '1px solid #d0d8e4', borderBottom: '1px solid #d0d8e4' }}>
