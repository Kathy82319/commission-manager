...
export function BasicInfoTab({ formData, setFormData, settings, setSettings }: Props) {
  ...
  const handleAvatarUpload = async (resultBlobs: { preview: Blob }) => {
    ...
    } catch (err: any) { alert(err.message || "頭像上傳失敗"); } 
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ borderTop: '1px solid #EAE6E1', paddingTop: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
          ...
          <button onClick={handleAddSocial} style={{ padding: '10px 20px', background: '#5D4A3E', color: '#FFF', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>+ 新增</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {settings.social_links.map((link, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#FAFAFA', borderRadius: '8px', border: '1px solid #EAE6E1' }}>
              <span style={{ fontSize: '14px' }}><strong style={{ color: '#5D4A3E' }}>{link.platform}:</strong> <span style={{ color: '#7A7269' }}>{link.url}</span></span>
              <button onClick={() => setSettings(prev => ({ ...prev, social_links: prev.social_links.filter((_, idx) => idx !== i) }))} style={{ color: '#A05C5C', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 'bold' }}>移除</button>
            </div>
          ))}
        </div>
      </div>
