...
export function ShowcaseTab({ onToggleGlobalSave, onToast, quotaInfo, isReadOnly }: ShowcaseTabProps) {
  ...
  useEffect(() => {
    onToggleGlobalSave(isBasicEditOpen || isBuilderOpen);
    return () => onToggleGlobalSave(false);
  }, [isBasicEditOpen, isBuilderOpen, onToggleGlobalSave]);
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', backgroundColor: '#FDFDFB' }}>
        <div style={{ padding: '16px 24px', background: '#FFFFFF', borderBottom: '1px solid #EAE6E1', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <button onClick={() => setIsBuilderOpen(false)} style={{ background: 'none', border: 'none', color: '#A0978D', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer' }}>⬅ 返回基本設定</button>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#5D4A3E', fontWeight: 'bold' }}>⚙️ 客製化表單與接單規則 ({editingItem.title || '未命名'})</h2>
          </div>
          <button onClick={handleSaveItem} style={{ padding: '10px 24px', background: '#4E7A5A', color: '#FFF', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px', boxShadow: '0 2px 8px rgba(78, 122, 90, 0.2)' }}>儲存所有變更</button>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
              ...
              <div style={{ background: '#FDFDFB', border: '1px solid #EAE6E1', borderRadius: '12px', padding: '20px' }}>
                ...
                {editingItem.max_orders > 0 && (
                  <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px dashed #DED9D3', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={handleResetOrdersCount} style={{ background: '#F4F4F1', color: '#A05C5C', border: '1px solid #DED9D3', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: 'bold' }}>↺ 重新計算收件數</button>
                  </div>
                )}
              </div>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '8px' }}>
                  ...
                  <button onClick={handleImportGlobalTOS} style={{ background: '#EBF2F7', color: '#4A7294', border: '1px solid #C1D6E8', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: 'bold' }}>⬇️ 一鍵帶入預設範本</button>
                </div>
              ...
              <div style={{ background: '#FAFAFA', border: '1px solid #DED9D3', borderRadius: '12px', padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <h4 style={{ margin: 0, color: '#5D4A3E', fontSize: '16px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>📋 客製化表單問題</h4>
                ...
                {showGuestWarning && (
                  <div style={{ background: '#FEF9C3', color: '#B45309', padding: '12px', borderRadius: '8px', border: '1px solid #FEF08A', fontSize: '13px', marginBottom: '16px', display: 'flex', gap: '8px' }}>
                    <span>⚠️</span><span>您開啟了訪客委託，但表單中似乎沒有詢問「聯絡方式」，這可能會導致您收到單後無法聯繫上對方喔！</span>
                  </div>
                )}
                    ...
                    <div key={field.id} style={{ background: '#FFF', border: '1px solid #EAE6E1', borderRadius: '8px', padding: '16px', position: 'relative', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                      ...
                      {['radio', 'checkbox', 'select'].includes(field.type) && (
                        <div style={{ marginTop: '12px', paddingLeft: '44px' }}>
                          <input className="form-input" style={{ background: '#FAFAFA' }} value={field.options?.join(', ') || ''} onChange={e => updateFormField(field.id, { options: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })} placeholder="設定選項 (用半形逗號 , 分隔) 例如：選項A, 選項B" />
                        </div>
                      )}
                    </div>
                  ))}
          ...
          <div className="custom-scrollbar" style={{ flex: '1', background: '#F4F0EB', padding: '40px', overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '100%', maxWidth: '450px', background: '#FFF', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', border: '1px solid #EAE6E1' }}>
              <div style={{ background: '#5D4A3E', color: '#FFF', padding: '12px', textAlign: 'center', fontSize: '13px', fontWeight: 'bold' }}>👁️ 委託人視角預覽</div>
              <div style={{ padding: '24px' }}>
                ...
                <div style={{ borderTop: '1px solid #EAE6E1', paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {formFields.map((field, idx) => (
                    <div key={idx}>
                      <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#5D4A3E', marginBottom: '8px' }}>{field.label || `未命名問題 ${idx+1}`} {field.required && <span style={{ color: '#E11D48' }}>*</span>}</div>
                      ...
                      {['radio', 'checkbox'].includes(field.type) && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          {(field.options?.length ? field.options : ['選項預覽']).map((opt, i) => (
                            <label key={i} style={{ fontSize: '13px', color: '#7A7269', display: 'flex', alignItems: 'center', gap: '6px' }}><input type={field.type} disabled /> {opt}</label>
                          ))}
                        </div>
                      )}
                    </div>
                  ))}
                  {formFields.length === 0 && <div style={{ color: '#A0978D', fontSize: '13px', textAlign: 'center', fontStyle: 'italic', padding: '20px 0' }}>無自訂表單，此項目為純作品展示</div>}
                </div>
                ...
                {editingItem.tos_content && (
                  <div style={{ marginTop: '24px', borderTop: '1px solid #EAE6E1', paddingTop: '20px' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#5D4A3E', marginTop: '8px', fontWeight: 'bold' }}><input type="checkbox" disabled /> 我已閱讀並同意上述協議</label>
                  </div>
                )}
                {formFields.length > 0 && <button disabled style={{ width: '100%', padding: '12px', background: '#5D4A3E', color: '#FFF', border: 'none', borderRadius: '8px', fontWeight: 'bold', marginTop: '24px', opacity: 0.7 }}>正式送出委託申請</button>}
              </div>
            </div>
          </div>
        </div>
  ...
  if (isBasicEditOpen) {
    return (
      <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', background: '#FFF', padding: '24px', borderRadius: '12px', border: '1px solid #EAE6E1' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #EAE6E1', paddingBottom: '16px' }}>
          <h3 style={{ margin: 0 }}>{editingItem.id ? '✏️ 編輯基本資料' : '✨ 新增項目'}</h3>
          <button onClick={() => setIsBasicEditOpen(false)} style={{ padding: '8px 16px', background: '#FAFAFA', border: '1px solid #DED9D3', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>取消並返回</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '2 1 400px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {/* 🌟 UX 改版：進階設定的入口 */}
            <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', padding: '20px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
              <div>
                <h4 style={{ margin: '0 0 4px 0', color: '#4A7294', fontSize: '15px' }}>進階：接單設定與專屬客製化表單</h4>
                <div style={{ fontSize: '13px', color: '#64748B' }}>目前狀態：{formFields.length > 0 ? `已啟用 (${formFields.length} 個問題)` : '純作品展示，未啟用表單'}</div>
              </div>
              <button onClick={() => setIsBuilderOpen(true)} style={{ padding: '10px 20px', background: '#4A7294', color: '#FFF', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 2px 4px rgba(74, 114, 148, 0.2)' }}>
                ⚙️ 設定客製表單與規則 ➔
              </button>
            </div>
          </div>
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0 }}>販售區管理 ({items.length} / {limit})</h3>
        <button onClick={openNewForm} disabled={items.length >= limit} style={{ padding: '10px 20px', background: items.length >= limit ? '#C4BDB5' : '#5D4A3E', color: '#FFF', border: 'none', borderRadius: '8px', cursor: items.length >= limit ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}>
          ...
          {items.map((item, index) => {
            ...
            return (
              <div key={item.id} style={{ border: '1px solid #EAE6E1', borderRadius: '12px', overflow: 'hidden', background: '#FFF', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                ...
                {isFull && (
                  <div style={{ position: 'absolute', top: '10px', right: '10px', background: '#EF4444', color: '#FFF', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', zIndex: 2 }}>🛑 已滿單</div>
                )}
                ...
                <div style={{ display: 'flex', borderTop: '1px solid #EAE6E1' }}>
                  <button onClick={() => openEditForm(item)} style={{ flex: 1, padding: '12px', background: '#FAFAFA', border: 'none', borderRight: '1px solid #EAE6E1', cursor: 'pointer', fontWeight: 'bold', color: '#5D4A3E' }}>編輯基本資料</button>
