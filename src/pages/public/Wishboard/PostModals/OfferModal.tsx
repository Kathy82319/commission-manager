...
export const OfferModal: React.FC<OfferModalProps> = ({
  ...
  const togglePortfolioImage = (imagePath: string) => {
    ...
    setForm((prev: any) => {
      ...
      if (exists) {
        ...
        if (prev.ref_images.length >= 5) return prev; // 最多 5 張
  ...
  const addQuestion = () => {
    ...
    setForm((prev: any) => ({ ...prev, questions: [...prev.questions, ''] }));
  };

  const updateQuestion = (index: number, value: string) => {
    const newQuestions = [...form.questions];
    newQuestions[index] = value;
    setForm((prev: any) => ({ ...prev, questions: newQuestions }));
  };
      ...
      <div className="post-modal">
        <div className="modal-header">
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <button type="button" onClick={onSaveDraft} className="save-hint-btn">
              <Save size={14} /> 設為預設
            </button>
        ...
        <form onSubmit={onSubmit} className="post-form">
          ...
          <div className="form-section">
            ...
            {/* 🌟 核心修正：使用正確的欄位 cover_url */}
            {showPortfolioPicker && (
              <div className="portfolio-picker-grid">
                {userShowcase
                  .filter((item: any) => item && item.cover_url) // 確保有圖片路徑
                  .map((item: any) => (
                  <div 
                    key={item.id} 
                    className={`portfolio-item ${form.ref_images.includes(item.cover_url) ? 'selected' : ''}`}
                    onClick={() => togglePortfolioImage(item.cover_url)}
                  >
                    <img src={getFullUrl(item.cover_url)} alt={item.title || "Portfolio"} />
                    {form.ref_images.includes(item.cover_url) && (
                      <div className="check-overlay"><Plus size={24} /></div>
                    )}
                  </div>
                ))}
              </div>
              ...
              {form.ref_images.length < 5 && (
                <div style={{ width: '120px', height: '120px', border: '2px dashed #cbd5e1', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <ImageUploader 
                    ...
                    targetWidth={1000} 
                    buttonText={isUploading ? "上傳中..." : "+ 上傳圖片"} 
                    maxSizeMB={3} 
                  />
                </div>
              )}
          ...
          <div className="form-section">
            ...
            <div className="dynamic-question-row">
              ...
              <input 
                ...
                style={{ flex: 1 }} 
              />
              <button type="button" onClick={addCommissionItem} className="add-btn-circle">
                <Plus size={18}/>
              </button>
            </div>
            
            <div className="item-manage-box">
              {form.commission_items.length === 0 && (
                <p style={{ fontSize: '13px', color: '#94a3b8', textAlign: 'center', margin: 0 }}>尚未新增接案項目</p>
              )}
              {form.commission_items.map((item: any, idx: number) => (
                <div key={idx} className="item-row">
                  <span style={{ fontWeight: '500', color: '#334155' }}>
                    {item.name} <span style={{ color: '#ff8c00', marginLeft: '8px' }}>${item.price}~</span>
                  </span>
                  <button type="button" onClick={() => removeCommissionItem(idx)} style={{ color: '#ef4444', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                    <Trash2 size={16}/>
                  </button>
                </div>
              ))}
          ...
          <div className="form-section selection-mechanism-box">
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div title="先搶先贏依順序額滿為止；選設制則由您挑選。" style={{ cursor: 'help', color: '#d97706' }}>
                <HelpCircle size={16}/>
              </div>
            </div>
            <div className="mechanism-radio-group">
              <label className="radio-label">
                <input 
                  type="radio" 
                  checked={form.selection_type === 'fcfs'} 
                  onChange={() => setForm({...form, selection_type: 'fcfs'})} 
                /> 先搶先贏
              </label>
              <label className="radio-label">
                <input 
                  type="radio" 
                  checked={form.selection_type === 'curated'} 
                  onChange={() => setForm({...form, selection_type: 'curated'})} 
                /> 繪師選設
              </label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: '10px' }}>
                ...
                <input 
                  ...
                  style={{ width: '80px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #fcd34d' }} 
                />
              </div>
            </div>
            <p style={{ fontSize: '13px', color: '#b45309', margin: '0' }}>
               {form.selection_type === 'curated' ? '💡 案主端將顯示：繪師會選擇適洽設定來接單' : '💡 案主端將顯示：剩餘名額 (滿額自動關閉)'}
            </p>
          </div>
          ...
          <div className="form-section">
            ...
            <div className="radio-group">
              {PAYMENT_TIMING.map(t => (
                <label key={t.value} className="radio-label">
                  <input 
                    type="radio" 
                    checked={form.payment_timing === t.value} 
                    onChange={() => setForm({...form, payment_timing: t.value})} 
                  /> {t.label}
                </label>
              ))}
            </div>
            {(form.payment_timing === 'deposit' || form.payment_timing === 'other') && (
              <input 
                type="text" 
                placeholder="請詳細描述您的收款時機... (例如：草稿確認後收50%訂金)" 
                value={form.payment_timing_detail} 
                onChange={e => setForm({...form, payment_timing_detail: e.target.value})} 
          ...
          <div className="form-section">
            ...
            {form.questions.map((q: string, idx: number) => (
              <div key={idx} className="dynamic-question-row">
                ...
                <button 
                  ...
                  style={{ border: 'none', background: 'none', color: '#94a3b8', cursor: 'pointer', padding: '8px' }}
                >
                  <Trash2 size={18}/>
                </button>
              </div>
            ...
            {form.questions.length < 3 && (
              <button type="button" onClick={addQuestion} className="add-btn-circle" style={{ width: 'fit-content', marginTop: '4px' }}>
                <Plus size={14} /> 新增題目
              </button>
            )}
          </div>
          ...
          <div className="form-section">
            ...
            <textarea 
              ...
              placeholder="請輸入您的版權說明、修改次數限制、退款政策等..." 
              value={form.tos_content} 
              onChange={e => setForm({...form, tos_content: e.target.value})}
            ></textarea>
          </div>
          ...
          <div className="form-section">
            ...
            <div className="tag-selector-group">
              ...
              <div className="tag-selector">
                {STYLE_WARNINGS.map(t => (
                  <span 
                    key={t} 
                    className={`selectable-tag warning ${form.tags.includes(t) ? 'selected' : ''}`} 
            ...
            <div className="tag-selector-group">
              ...
              <div className="tag-selector">
                {LICENSE_TAGS.map(t => (
                  <span 
                    key={t} 
                    className={`selectable-tag license ${form.tags.includes(t) ? 'selected' : ''}`} 
                    onClick={() => toggleTag(t, 'tags')}
                  >
                    {t}
                  </span>
            ...
            <div className="tag-selector-group">
              ...
              <div className="tag-selector">
                ...
                {form.tags.filter((t: string) => !REQ_TAGS.includes(t) && !STYLE_WARNINGS.includes(t) && !LICENSE_TAGS.includes(t)).map((t: string) => (
          ...
          <div className="form-section">
            ...
            <textarea 
              rows={4} 
              className="detail-textarea" 
              placeholder="除了上述設定外，還有其他想補充的細節嗎？"
              value={form.content} 
