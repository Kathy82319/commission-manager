...
import { REQ_TAGS, PAY_TAGS, STYLE_WARNINGS, LICENSE_TAGS, PAYMENT_TIMING, R2_PUBLIC_URL } from '../constants';
...
export const OfferModal: React.FC<OfferModalProps> = ({
  ...
  const removeImage = (index: number) => {
    setForm((prev: any) => ({ ...prev, ref_images: prev.ref_images.filter((_: any, i: number) => i !== index) }));
  };

  const togglePortfolioImage = (imageKey: string) => {
    setForm((prev: any) => {
      if (prev.ref_images.includes(imageKey)) return { ...prev, ref_images: prev.ref_images.filter((img: string) => img !== imageKey) };
  ...
  const addCommissionItem = () => {
    if (!itemInput.name.trim() || !itemInput.price) return;
    setForm((prev: any) => ({ ...prev, commission_items: [...prev.commission_items, { ...itemInput }] }));
    ...
    {userShowcase
      ...
      >
        ...
        {form.ref_images.includes(item.image_key) && <div className="check-overlay"><Plus size={24} /></div>}
            ...
            <div className="form-group" style={{ flex: '1 1 300px' }}>
              ...
              <div className="radio-group">
                ...
                {form.schedule_type === 'fixed' && (
                  <input type="date" className="date-input" value={form.specific_date} onChange={e => setForm({...form, specific_date: e.target.value})} required />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="form-section">
            <label className="section-title">接案項目與底價 (選填，展示於卡片)</label>
            ...
            <div className="item-manage-box" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: '8px' }}>
              {form.commission_items.length === 0 && <p style={{ fontSize: '13px', color: '#94a3b8', margin: 0 }}>尚未新增項目</p>}
              {form.commission_items.map((item: any, idx: number) => (
                <div key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                  <span style={{ fontWeight: '500' }}>{item.name}</span>
                  <span style={{ color: '#ff8c00', fontWeight: 'bold' }}>${item.price}</span>
                  <X size={14} color="#ef4444" style={{ cursor: 'pointer' }} onClick={() => removeCommissionItem(idx)} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="form-section">
            ...
            <textarea rows={4} className="detail-textarea" placeholder="請輸入您的版權說明、修改次數限制、退款政策等..." value={form.tos_content} onChange={e => setForm({...form, tos_content: e.target.value})}></textarea>
          </div>
